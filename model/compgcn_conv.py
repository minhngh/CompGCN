...
class CompGCNConv(MessagePassing):
	...
	def forward(self, x, edge_index, rel_emb): 
		...
		out_res		= self.propagate('add', self.out_index,  x=x, rel_emb=rel_embed, edge_norm=self.out_norm,	mode='out')
		out		= self.drop(out_res)

		if self.p.bias: out = out + self.bias
		out = self.bn(out)

		return self.act(out), torch.matmul(rel_embed, self.w_rel)		# Ignoring the self loop inserted
	...
	def message(self, x_j, rel_emb, edge_norm, mode):
